...
| (0) Export the simulator app as **ARM64** (`PlayerSettings.iOS.simulatorSdkArchitecture`) in Testbed2022 and Testbed6 | **done** (2026-09-16) | the exports were x86_64-only because only the *device* architecture was set; set in `TestbedBuilder.BuildSimulator` behind `#if UNITY_2022_1_OR_NEWER`, not in the checked-in asset, because every Library cache key hashes `ProjectSettings/**`. ARM64 rather than Universal: Universal compiles both slices and doubles the il2cpp phase. §6.3 |
