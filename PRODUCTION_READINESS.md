...
| iOS app-delegate lifecycle (no scene manifest) — `UnityAppController` swizzle + cold/warm capture | review + **CI mock-host coexistence leg (Testbed2022 2022.3.62f3, iOS Simulator)** | ✅ **CI** — `ios-simulator-coex (2022.3-coex)` launches the manifest-less export under a mock host that is at once an `IMPL_APP_CONTROLLER_SUBCLASS` subclass, a vendor-style category swizzle and a GoogleUtilities-style isa proxy, and requires by name: the package's install line, `lifecycle-app-delegate`, the cold launch item queued exactly once with `NO` returned and the host's discarded `NO` not double-delivering, the warm redelivery of that item collapsed by the consume-once marker with its completion run once, a warm tap through the proxied delegate queued once with one completion, and an unmarked item's completion run once. Its first run (2026-09-02) also measured that the mock's category `+load` ran BEFORE the package's class `+load` on this export (after it on Unity 6), so the leg records the order and asserts composition in either. Synthetic sends there; the `ios-springboard (2022.3)` leg adds SpringBoard's own tap on the same export — run 77 (2026-09-15): an XCUITest bundle long-pressed the icon, tapped the context-menu row `daily_reward`, the app cold-started and the id reached `Performed` (the testbed's marker file) 35–43 s after the touch, iOS 18.6; run 78, with the 120 s window, `PASS` (41 s); run 94 added a second tap per leg on `runtime_add`, a row `QuickActions.Add` published at runtime rather than one `Info.plist` baked — `PASS` 5 s after the tap on iOS 26.2, with all four rows (three static, one runtime) in the one menu SpringBoard opened. The older "Unity 6.3" Simulator run is credited to neither lifecycle (no patch number, `Info.plist` never inspected). ⏳ **device** — physical-hardware confirmation still open |
...
| Static-shortcut `{placeholder}` interpolation + build-code hooks (`QuickActionsStaticBuild`: `Customize`, `RegisterPlaceholder`) | unit + static | 17 headless tests pin the engine — escaping, case-insensitivity, unknown-token verbatim, override/custom precedence, per-locale rows, throwing resolvers, the customizer contract, copy-not-mutate on subscriber items — and both bakers consume the prepared list. The resolved strings ride the two bake paths proven above/below. Since run 100 (2026-09-17) the demo carries one — `Continue`'s subtitle is `Resume v{version} ({build})` in every testbed — and three CI checks read the resolved `Resume v1.4.0 (37)` back: the exported `Info.plist` (all three Unity lines, and it also rejects a raw brace in any baked label), the APK resource table via `aapt2` (all four Android legs), and the labels of the menu SpringBoard opens. Both platforms **render** it: the emulator popup drew it on 2021.3/2022.3/Unity 6, SpringBoard's menu listed `Continue, Resume v1.4.0 (37)` on iOS 26.2 and 26.5. ⏳ **device** — no handset or iPhone has shown it. |
...
| Android: trampoline `<activity>` absent when OFF (never injected + stripper defense-in-depth) | review + **CI define-off APK** + earlier hand-run builds | ✅ **CI** — the same `gate-off` job builds a define-OFF IL2CPP APK in `android-build`'s exact configuration and requires: no trampoline `<activity>` and no `android.app.shortcuts` in the manifest, no `quickactions_*` / `qa_*` / `ic_quickaction_builtin_*` resources, nothing of the package's assembly (image name, namespaces) in the IL2CPP metadata (the define-ON APK is the positive control) — then diffs the two APKs entry by entry (plus the whole-file size), which is the package's **measured footprint**, printed in the job summary and capped at 1 MiB — 144,061 bytes (140.7 KiB) compressed on run 52's APK pair (2022.3, IL2CPP arm64+armv7): two IL2CPP libraries, 31 KiB of metadata, 47 KiB of `libunity.so`, ~10 KB of resources. Hand-proven before CI: real prod Gradle build (define OFF) on 2022.3, re-proven on 2021.3.45f2 and 6000.3.21f1 (2026-07-17). Since 0.7.0 the two plugin `.java` classes (trampoline + bridge, ~20 KB of bytecode) are **gated too**: the ungated stripper deletes the sources Unity stages at `unityLibrary/src/main/java/com/emindeniz99/quickactions/` before Gradle reads the source set, and the job requires **0** dex references to that package on the define-off APK (the define-on count is the control). Through 0.6.x they remained as dead code unless R8 minified them, counted rather than gated. |
| Release-configuration player: Managed Stripping Level **High** + R8 `minifyEnabled` | **CI (`2022.3-release` build + smoke legs)** | ✅ **CI** — first run 2026-09-02 (PR #19 run 64), green. A fourth `android-build` matrix leg builds Testbed2022 through `TestbedBuilder.BuildAndroidPhoneRelease`: no `BuildOptions.Development`, `SetManagedStrippingLevel(Android, High)`, `PlayerSettings.Android.minifyRelease = true`, and the keep rule README.md documents — `-keep class com.emindeniz99.quickactions.** { *; }` in `Assets/Plugins/Android/proguard-user.txt`, checked into all three testbeds, with Publishing Settings ▸ Custom Proguard File (`useCustomProguardFile`) switched on by that same build method — so what is under test is the documented recipe rather than a CI-only variation of it. What that run's APK showed: UnityLinker ran with `--rule-set=Experimental` (the High rule set) and `global-metadata.dat` went from 1,700,348 to 1,176,152 bytes; R8 ran — the development build's four `classes*.dex` collapsed into one, with 15 one-or-two-letter class names where the development APK has none — while `QuickActionsBridge`, `QuickActionsBridge$1` and `QuickActionsTrampolineActivity` kept their names; the APK came out at 14,183,406 bytes against the development build's 23,690,490. The minified APK passed the identical aapt2/manifest/dex assertions, and `android smoke (2022.3-release)` then ran it on the API 30 emulator: `AUTOTEST=add3` registered three shortcuts, and a warm tap plus a post-`force-stop` cold tap each came back as `Performed`. So the documented recipe holds under R8, and stripping High removed nothing the `[Serializable]` DTOs (`QuickActionList` / `QuickActionItem`) need — no `link.xml` was required. Not covered: whether the keep rule is *necessary* (nothing builds minified without it), resource shrinking (`shrinkResources` stays off — that is `android-shrink-verify`'s question), and physical hardware. |
