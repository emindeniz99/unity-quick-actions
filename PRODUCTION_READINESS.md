...
**Legend — Verified by:**
`unit` = headless NUnit (`dotnet test`, 111 tests) · `unity-test` = Unity Test
...
| Built-in Android icons → drawable-xhdpi/ic_quickaction_{add,compose,favorite,play}.png | unit + static + CI (aapt2 + shrinker) | ⏳ **device** — 11 harness tests pin the byte-identical write, the dynamic-only and no-launcher cases, idempotence, the project-drawable override (nested under unityLibrary, at the Gradle root, or via a settings.gradle projectDir; any extension/density) and the define-off sweep; every CI APK is read back with aapt2 for all four on all three lines, and the shrink job requires them byte-identical after `shrinkResources`. Never seen on a launcher: the README screenshot predates them. |
