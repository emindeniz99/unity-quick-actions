...
| Release-configuration player: Managed Stripping Level **High** + R8 `minifyEnabled` | **CI (`2022.3-release` build + smoke legs) — added, never run** | ⏳ **CI leg added, first run pending.** A fourth `android-build` matrix leg builds Testbed2022 through `TestbedBuilder.BuildAndroidPhoneRelease`: no `BuildOptions.Development`, `SetManagedStrippingLevel(Android, High)`, `PlayerSettings.Android.minifyRelease = true`, and the keep rule README.md documents — `-keep class com.emindeniz99.quickactions.** { *; }` in `Assets/Plugins/Android/proguard-user.txt`, checked into all three testbeds, with Publishing Settings ▸ Custom Proguard File (`useCustomProguardFile`) switched on by that same build method — so what is under test is the documented recipe rather than a CI-only variation of it. The minified APK gets the identical aapt2/manifest/dex assertions (the dex class names are what R8 was free to rename), and `android smoke (2022.3-release)` then runs it on the API 30 emulator: `AUTOTEST=add3` must register three shortcuts and a warm tap plus a post-`force-stop` cold tap must each come back as `Performed`. That is the proof both halves were missing — R8 must not have renamed the JNI-by-name `QuickActionsBridge`, and stripping High must not have removed anything the `[Serializable]` DTOs (`QuickActionList` / `QuickActionItem`) need, the package shipping no `link.xml`. Everything above is what the leg *does*: **it has not run yet, so no green is claimed here.** Not covered either way: whether the keep rule is *necessary* (nothing builds minified without it), resource shrinking (`shrinkResources` stays off — that is `android-shrink-verify`'s question), and physical hardware. |
