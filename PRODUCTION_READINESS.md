...
**Legend — Verified by:**
`unit` = headless NUnit (`dotnet test`, 98 tests) · `unity-test` = Unity Test
...
| Icon keep rule → res/raw/quickactions_keep.xml (resource shrinker) | unit + static + review | ⏳ **build** — 8 headless tests pin that it is written on every enabled Android build (zero static shortcuts included), its parsed tools:keep value, idempotence, and that the static-shortcut cleanup leaves it alone; that the shrinker honors it has NOT been run on a real minified build (ROADMAP verification item (c)). |
