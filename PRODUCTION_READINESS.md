...
**Legend — Verified by:**
`unit` = headless NUnit (`dotnet test`, 117 tests) · `unity-test` = Unity Test
...
- **Headless gate (closable without a Unity Editor): GREEN.** `tools~/verify.sh` → **VERIFY: PASS** —
  10 C# configs compile with **0 warnings**, **117 unit tests pass** (`dotnet test`),
...
- **Test inventory (where each number comes from).** 123 distinct C# tests:
  ...
  - **46 headless-only** — `.verify/EditorTests/AndroidStaticLocalizationTests.cs`
    (4), `AndroidKeepRulesTests.cs` (8), `AndroidBuiltInIconsTests.cs` (12),
  ...
  So `dotnet test` reports **117** (71 + 46) and a Unity Test Runner run reports
