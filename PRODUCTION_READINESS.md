...
| iOS app-delegate lifecycle (no scene manifest) — `UnityAppController` swizzle + cold/warm capture | review + an iOS Simulator run whose lifecycle went unrecorded | ⏳ **not attributable** — the one run against real UIKit (Simulator, "Unity 6.3" / iOS 26.5) did show a home-screen shortcut cold-launching the app and the action id arriving on the C# `Performed` event (SpringBoard's log and the app console), but it was logged without an editor patch number and its exported `Info.plist` was never inspected, so it cannot be credited to this lifecycle rather than the UIScene one below — the only 6.3 project here is `Examples~/Testbed6` on 6000.3.21f1, past the 6000.3.8f1 line where the scene manifest appears. What would turn this row ✅ is a tap run on an export without a scene manifest (Testbed2021 or Testbed2022) with its `Info.plist` kept. ⏳ **device** — physical-hardware confirmation still open |
| iOS UIScene lifecycle (2022.3.72f1+ / 6000.0.68f1+ / 6000.3.8f1+) — scene-delegate discovery + cold/warm capture | review only | ⏳ **unexercised** — no Simulator or device run has been shown to take this path. The `ios-simulator` CI leg builds and launches a scene-manifest project (Testbed6, 6000.3.21f1) but asserts only that the process survives, which stays true for an app whose scene connected with no delegate. The mock-host coexistence leg is the first check that asserts on the hooks; it has not run yet |
